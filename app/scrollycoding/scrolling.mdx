...
## !!steps 02a: a table to store notes
...
## !!steps 02b: POST /api/notes to create notes
### 02a -> 02b - changes in <code>main.py</code>
#### an API endpoint to create notes
...
## !!steps 02c: GET /api/notes endpoint
...
## !!steps 02d: GET /api/notes/id endpoint
### 02c -> 02d - changes in <code>main.py</code>
...
## !!steps 03a: expose static files
### 02d -> 03a - changes in <code>main.py</code>
...
- the payload is implicitly assumed to be exposed in the Body part of the incoming request
- here again we use the `Note` class to define the type of that payload; this means that
...
## !!steps 06: clicking done is published to the backend
...
## !!steps 07 (1/2): the UI can create a note
...
## !!steps 07 (2/2): the UI can create a note
...
## !!steps 09 (1/2): the UI can delete a note
...
## !!steps 09 (2/2): the UI can delete a note
...
## !!steps 10a: fine-grained types NoteCreate NoteUpdate Note
...
## !!steps 11a (1/2): the WebsocketBroadcaster class
...
## !!steps 11a (2/2): the WebsocketBroadcaster class
...
## !!steps 11c: inject ws-listener.js in template
...
## !!steps 12: the api endpoints broadcast the changes on WS
...
## !!steps 13a (1/2): handle incoming WS messages in the frontend
...
## !!steps 13a (2/2): handle incoming WS messages in the frontend
...
## !!steps 13b: define id=note-{id} to the note &lt;li&gt;
...
## !!steps 13c: CSR on note creation
...
## !!steps 13e: CSR on note deletion
