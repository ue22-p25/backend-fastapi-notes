...
STEPS_REPO=git@github.com:ue22-p25/backend-fastapi-notes-steps.git
...
function toauto-1() {
    ...
    python $BIN/stepstohike.py chain-dirs --all-files --only-changes-with-step ${STEPS}/.steps/* > $out
    echo single column output written in $out
}
# compute AUTO in scrollycoding
function toauto-2() {
    local out=$APP/scrollycoding/AUTO
    python $BIN/stepstohike.py chain-dirs --all-files --only-changes-with-step --scrolly ${STEPS}/.steps/* > $out
...
function testauto-1() {
    local out=$APP/singlecolumn/AUTO
    python $BIN/stepstohike.py chain-dirs --all-files --only-changes-with-step $TESTSTEPS > $out
    echo single column output written in $out
}
function testauto-2() {
    local out=$APP/scrollycoding/AUTO
    python $BIN/stepstohike.py chain-dirs --all-files --only-changes-with-step --scrolly $TESTSTEPS > $out
    echo double column output written in $out
}
function testauto() {
    testauto-1
